...
const isLoginValid = (login) => {
  ...
  return login.length >= minLenght && login.length <= maxLenght;
};
...
const addLogin = (allLogins, login) => {
  ...
  if (!isValid) {
    ...
    return hadleValidLogin(allLogins, login);
  } 
};

const hadleValidLogin = (allLogins, login) => {
  const isUnique = isLoginUnique(allLogins, login);
  
      if (!isUnique) {
        return showIsLoginNotUnique();
      } else {
        allLogins.push(login);
        return showLoginAddedMessage();
      }
}
...
const showIsLoginNotUnique = () => {
   return 'Такой логин уже используется!';
}

const showLoginAddedMessage = () => {
